...
actor {
    ...
    public shared query ({caller}) func getAllPollsWithYesNoAnswers() : async [(Poll, (Nat, Nat), Bool)] {
        //no time to get propper types   
        let polls = _getAllPolls();
        let buf = Buffer.Buffer<(Poll, (Nat, Nat), Bool)>(8);
        ...
        for (poll in Iter.fromArray(polls)) {
            ...
            buf.add(poll , answerScore, answerAlready);
        };
